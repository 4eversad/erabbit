...
export default {
  name: "XtxCartPage",
  components: { GoodRelevant, CartNone, XtxConfirm, CartSku },

  setup(props, { emit }) {
    const store = useStore();
    const checkOne = (skuId, selected) => {
      console.log(skuId, selected);
      store.dispatch("cart/updateCart", { skuId, selected }, { root: true});
    };
    const checkAll = (selected) => { 
      store.dispatch("cart/checkAllCart", selected, checkAll, { root: true});
    };
    const deleteCart = (skuId) => {
      Confirm({ text: "确认删除该商品吗?" })
        .then(() => {
    ...
    const batchDeleteCart = (isClear) => {
      Confirm({ text: `"确认删除${isClear ? "无效" : "选中"}的商品吗?"` })
        .then(() => {
          console.log(123);
    ...
    // 修改规格信息
    const updateCartSku = (oldSkuId, newSkuInfo) => {
    ...
    const checkout = () => {
      ...
      if (store.getters["cart/selectedList"].length === 0)
        return Message({ message: "请至少勾选一件商品" });
